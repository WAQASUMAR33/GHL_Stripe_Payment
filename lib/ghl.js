...
export async function exchangeGHLCode(code) {
  ...
  const { data } = await axios.post(`${GHL_OAUTH_BASE}/token`, params.toString(), {
...
export async function refreshGHLToken(locationId) {
  const stored = getGHLTokens(locationId);
  ...
  saveGHLTokens(locationId, updated);
  return updated;
}
...
export async function getValidAccessToken(locationId) {
  let tokens = getGHLTokens(locationId);
