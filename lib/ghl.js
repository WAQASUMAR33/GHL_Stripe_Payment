...
export async function postPaymentUpdateToGHL(locationId, payload) {
  ...
  const body = {
    ...
    ...payload,
    // Override amount with dollar value and use GHL's expected field name for entity type
    ...(amountDollars !== undefined ? { amount: amountDollars } : {}),
    ...(payload.entityType ? { entitySourceType: payload.entityType } : {}),
  };
  delete body.entityType; // GHL uses entitySourceType
