...
export async function createGHLPaymentProvider(locationId) {
  ...
  const client = await ghlClient(locationId);
  const { data } = await client.post('/payments/custom-provider/provider', {
    locationId,
    name:        'Stripe',
 ...
 */
export async function connectGHLPaymentProvider(locationId, liveMode = false) {
  // Creating the provider is sufficient to register it in GHL.
  // The separate /connect call is attempted as a best-effort bonus.
  ...
  try {
    ...
    // Connect call is optional — provider is already registered via createGHLPaymentProvider
    console.warn(`[GHL] connect call failed (non-fatal): ${err.response?.status} ${JSON.stringify(err.response?.data ?? err.message)}`);
    return providerData;
  }
}
