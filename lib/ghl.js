...
export async function postPaymentUpdateToGHL(locationId, payload) {
  ...
  // GHL expects amount in dollars, Stripe stores in cents
  const amountDollars = payload.amount != null ? payload.amount / 100 : undefined;

  // GHL only accepts specific entitySourceType values
  const GHL_VALID_SOURCE_TYPES = new Set(['funnel', 'website', 'invoice', 'store', 'membership', 'calendar', 'subscriptions', 'order_form']);
  const rawEntityType = payload.entityType ?? '';
  const entitySourceType = GHL_VALID_SOURCE_TYPES.has(rawEntityType.toLowerCase())
    ? rawEntityType.toLowerCase()
    : undefined;
  ...
  const body = {
    ...
    altType:            'location',
    locationId,
    marketplaceAppId:   process.env.GHL_PAYMENT_PROVIDER_ID ?? process.env.GHL_CLIENT_ID,
    apiKey:             process.env.GHL_CLIENT_SECRET,
    ...payload,
    // GHL expects amount in dollars; use valid entitySourceType or omit it
    ...(amountDollars !== undefined ? { amount: amountDollars } : {}),
    ...(entitySourceType ? { entitySourceType } : {}),
  };
  delete body.entityType; // replaced with entitySourceType above
  console.log('[GHL] postPaymentUpdateToGHL body:', JSON.stringify(body));
  const response = await axios.post(
    `${GHL_API_BASE}/payments/custom-provider/webhook`,
    body,
    {
      headers: {
        Authorization:  `Bearer ${accessToken}`,
